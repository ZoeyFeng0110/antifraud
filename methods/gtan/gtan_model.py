...
def info_nce(view1, view2, temperature, label, b_cos=True):
    if b_cos:
        view1, view2 = F.normalize(view1, dim=1), F.normalize(view2, dim=1)

    # 对标签按0/1进行分类，获取对应下标
    idx_label_0 = (1 - label).nonzero(as_tuple=True)[0]
    idx_label_1 = label.nonzero(as_tuple=True)[0]
    # 采样，使得 idx_label_0 的数量与 idx_label_1 一致
    perm = torch.randperm(idx_label_0.size(0))
    idx = perm[:int(idx_label_1.size(0))]
    idx_label_0 = idx_label_0[idx]
    idx = torch.cat([idx_label_0, idx_label_1])  # 合并索引
    idx = torch.unique(idx)  # 去重

    # 选择 idx对应下标的值作为新的 view1,view2
    view1 = view1[idx]
    view2 = view2[idx]
...
class CLEncoder(nn.Module):
    ...
    def __init__(self, eps, temp, layer_num, input_dim, hidden_dim, output_dim, dropRate, relu_rate):
        ...
        self.layers = nn.ModuleList(
            [nn.Linear(input_dim if i == 0 else hidden_dim, hidden_dim).cuda() for i in range(layer_num)])
        self.batch_norms = nn.ModuleList(
            [nn.BatchNorm1d(hidden_dim).cuda() for _ in range(layer_num)])  # BatchNorm Layers
        self.output_layer = nn.Linear(hidden_dim, output_dim).cuda()

    def forward(self, features, noise_indices=[], train_flag=False):
        # print(features.shape)
        batch_features = features
        all_embeddings = []
        ego_embeddings = batch_features
        for i, layer in enumerate(self.layers):
            ego_embeddings = layer(ego_embeddings)
            ego_embeddings = self.batch_norms[i](ego_embeddings)  # Applying BatchNorm Layer
            ego_embeddings = F.dropout(ego_embeddings, self.dropRate, train_flag)
            ego_embeddings = nn.LeakyReLU(self.relu_rate)(ego_embeddings)
            if train_flag:
                noise_mask = torch.ones(ego_embeddings.size(0), dtype=torch.bool)
                noise_mask[noise_indices] = False
                noise_mask = noise_mask.cuda()
                random_noise = torch.rand_like(ego_embeddings).cuda()
                # ego_embeddings += torch.sign(ego_embeddings) * F.normalize(random_noise, dim=-1) * self.eps
                ego_embeddings[noise_mask] += torch.sign(ego_embeddings[noise_mask]) * F.normalize(
                    random_noise[noise_mask], dim=-1) * self.eps
            all_embeddings.append(ego_embeddings)
        # Last layer
        ego_embeddings = self.output_layer(ego_embeddings)
        ego_embeddings = F.dropout(ego_embeddings, self.dropRate, train_flag)
        ego_embeddings = nn.LeakyReLU(self.relu_rate)(ego_embeddings)
        if train_flag:
            random_noise = torch.rand_like(ego_embeddings).cuda()
        ...
        return final_embeddings, all_embeddings_cl

    def loss(self, features, labels, noise_indices, train_flag):
        all_embedding, all_embedding_cl = self.forward(features, noise_indices, train_flag)
        return info_nce(all_embedding, all_embedding_cl, self.temp, labels)
...
class GraphAttnModel(nn.Module):
    ...
    def forward(self, blocks, features, labels, n2v_feat=None, train_flag=False):
        """
        ...
        :param n2v_feat: whether to use n2v features 
        """
        features = self.cl_encoder(features, train_flag)[0]
        if n2v_feat is None:
            h = features
        else:
    ...
    def cl_loss(self, features, labels, train_flag=False):
        return self.cl_encoder.loss(features, labels, train_flag)
