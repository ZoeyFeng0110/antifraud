...
class Jensen_Shannon(nn.Module):
    ...
    def calculate_JSD(self, batch_logits, batch_labels):
        batch_logits = torch.sigmoid(batch_logits)
        # Divergence clculator to record the diff. between ground truth and output prob. dist.
        dist = self.calculate(batch_logits, F.one_hot(batch_labels.long(), num_classes=2))
        return dist

    def evel_by_jsd(self, batch_logits, batch_labels):
        with torch.no_grad():
            class_ind = {}
            num_sample = batch_logits.shape[0]
            prob = self.calculate_JSD(batch_logits, batch_labels)
            threshold = torch.mean(prob)
            if threshold.item() > self.d_u:
                threshold = threshold - (threshold - torch.min(prob)) / self.tau
            SR = torch.sum(prob < threshold).item() / num_sample
            print(f"threshold: {threshold}, sample ratio: {SR}")
            for kk in range(2):
                class_ind[kk] = [i for i, x in enumerate(batch_labels) if x == kk]
            pred_idx = np.zeros(int(SR * num_sample))
            class_len = int(SR * num_sample / 2)
            size_pred = 0
            # Ranking-based Selection and Introducing Class Balance
            for i in range(2):
                class_indices = class_ind[i]
            ...
            print(f"noisy number: {len(pred_idx_noisy)}, clean number: {len(pred_idx)}")
            return pred_idx_noisy
