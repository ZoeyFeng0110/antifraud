...
from methods.gtan.gtan_model import CLEncoder
from methods.gtan.losses import *


def generate_noisy_label(labels, r=0):
    size = labels.shape[0]
    num_noise = int(r * size)
    # 随机选择索引
    noise_indices = torch.randperm(size)[:num_noise]
    # 在选定的索引位置添加噪声
...
def gtan_main(feat_df, graph, train_idx, test_idx, labels, args, cat_features):
    print(f"dataset: {args['dataset']}, noisy rate: {args['noisy_rate']}")
    device = args['device']
    ...
    loss_all = nn.CrossEntropyLoss(reduction='none').to(device)
    # loss_fn = NCEandRCE(1, 1, 2)
    for fold, (trn_idx, val_idx) in enumerate(kfold.split(feat_df.iloc[train_idx], y_target)):
        ...
        test_dataloader_v2 = NodeDataLoader(graph,
                                            test_ind,
                                            test_sampler,
                                            use_ddp=False,
                                            device=device,
        ...
        # TODO
        cl_encoder = CLEncoder(eps=0.15, temp=1.23, layer_num=3,
                               input_dim=feat_df.shape[1],
                               hidden_dim=feat_df.shape[1],
                               output_dim=feat_df.shape[1],
                               dropRate=0.3,
                               relu_rate=0.1)
        model = GraphAttnModel(in_feats=feat_df.shape[1],
                               ...
                               cat_features=cat_feat, encoder=cl_encoder).to(device)
        lr = args['lr'] * np.sqrt(args['batch_size'] / 1024)  # 0.00075
        optimizer = optim.Adam(model.parameters(), lr=lr,
                               weight_decay=args['wd'])
        optimizer_cl = optim.Adam(cl_encoder.parameters(), lr=lr,
                                  weight_decay=args['wd'])
        ...
        # for _, param in enumerate(cl_encoder.state_dict()):
        #     print(param)
        #     print('----------------')
        js = Jensen_Shannon(0.7, 5)
        for epoch in range(start_epoch, args['max_epochs']):
            ...
            if epoch <= 10:
                for step, (input_nodes, seeds, blocks) in enumerate(train_dataloader):
                    ...
                    # train_batch_logits = model(blocks, batch_inputs, lpa_labels, batch_work_inputs, True)
                    mask = batch_labels == 2
                    # train_batch_logits = train_batch_logits[~mask]
                    batch_labels = batch_labels[~mask]
                    batch_labels, noise_indices = generate_noisy_label(batch_labels, args['noisy_rate'])
                    train_loss = cl_encoder.loss(batch_inputs, batch_labels, noise_indices, True)
                    # backward
                    ...
                    # lr_scheduler.step()
                    if step % 2 == 0:
                        print(f'In epoch:{epoch}|batch:{step}, train_loss:{train_loss.item()}, ')
            else:
                for step, (input_nodes, seeds, blocks) in enumerate(train_dataloader):
                    batch_inputs, batch_work_inputs, batch_labels, lpa_labels = load_lpa_subtensor(num_feat, cat_feat,
                                                                                                   labels,
                    ...
                    with torch.no_grad():
                        noise_indices = js.evel_by_jsd(train_batch_logits, batch_labels)
                        # losses = loss_all(train_batch_logits, batch_labels)
                        # losses = (losses - losses.min()) / (losses.max() - losses.min())
                        # input_loss = losses.reshape(-1, 1).cpu().numpy()
                        # # fit a two-component GMM to the loss
                        # gmm = GaussianMixture(n_components=2, max_iter=20, tol=1e-3, reg_covar=1e-6)
                        # gmm.fit(input_loss)
                        # prob = gmm.predict_proba(input_loss)
                        # prob = prob[:, gmm.means_.argmin()]
                        # pred = (prob > 0.5)
                        # noise_indices = (1 - pred).nonzero()[0]

                    train_loss = loss_fn(train_batch_logits, batch_labels) + args['lambda_cl'] * cl_encoder.loss(
                        batch_inputs, batch_labels, noise_indices, True)
                    ...
                    if step % 10 == 0:
                        ...
                        batch_labels_list = batch_labels.cpu().numpy()
            ...
            with torch.no_grad():
                for step, (input_nodes, seeds, blocks) in enumerate(val_dataloader):
                    ...
                    if step % 10 == 0:
                        ...
                        score1 = torch.argmax(val_batch_logits.clone().detach(), dim=1)[:].cpu().numpy()
                        batch_labels_list = batch_labels.cpu().numpy()

                        print('In epoch:{:03d}|batch:{:04d}, val_loss:{:4f}, val_ap:{:.4f}, '
                              'val_acc:{:.4f}, val_auc:{:.4f} , val_f1:{:.4f}'.format(epoch,
                                                                                      step,
                                                                                      ...
                                                                                      f1_score(
                                                                                          batch_labels_list,
                                                                                          score1)))
                for step, (input_nodes, seeds, blocks) in enumerate(test_dataloader_v2):
                    batch_inputs, batch_work_inputs, batch_labels, lpa_labels = load_lpa_subtensor(num_feat, cat_feat,
                                                                                                   labels,
                                                                                                   seeds, input_nodes,
                                                                                                   device)
                    ...
                    blocks = [block.to(device) for block in blocks]
                    test_batch_logits = model(blocks, batch_inputs, lpa_labels, batch_work_inputs)
                    mask = batch_labels == 2
                    test_batch_logits = test_batch_logits[~mask]
                    batch_labels = batch_labels[~mask]
                    test_batch_pred = torch.sum(torch.argmax(
                        test_batch_logits, dim=1) == batch_labels) / torch.tensor(batch_labels.shape[0])
                    test_acc_list = test_acc_list + test_batch_pred * \
                                    torch.tensor(batch_labels.shape[0])
                    test_all_list = test_all_list + batch_labels.shape[0]
                    if step % 10 == 0:
                        score = torch.softmax(test_batch_logits.clone().detach(), dim=1)[:, 1].cpu().numpy()
                        score1 = torch.argmax(test_batch_logits.clone().detach(), dim=1)[:].cpu().numpy()
                        batch_labels_list = batch_labels.cpu().numpy()

                        print('In epoch:{:03d}|batch:{:04d}, test_ap:{:.4f}, '
                              'test_acc:{:.4f}, test_auc:{:.4f} , test_f1:{:.4f}'.format(epoch,
                                                                                         step,
                                                                                         average_precision_score(
                                                                                             batch_labels_list,
                                                                                             score),
                                                                                         test_batch_pred.detach(),
                                                                                         roc_auc_score(
                                                                                             batch_labels_list,
                                                                                             score),
                                                                                         f1_score(
...
def load_gtan_data(dataset: str, test_size: float):
    ...
    prefix = "/home/zjzhou/model/Baseline/GTAN/data/"
    if dataset == "S-FFSD":
